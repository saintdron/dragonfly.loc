...
const dotsCount = 250; // Amount of points
const neighborsCount = 10; // Number of links
const maxRange = 6000; // Maximum line length
const showDots = true; // Maximum line length

let canvas, c, fillColor,
    dots = new Array(dotsCount);

window.onload = function () {
    canvas = document.getElementById("lineGeneratorField");
    c = canvas.getContext("2d");
    fillColor = window.getComputedStyle(canvas).backgroundColor;
    canvas.width = document.body.clientWidth;
    canvas.height = 250;
    c.lineWidth = 0.2;
    c.strokeStyle = "#fff";
    getStartCoords();
    setInterval(newCoords, 30);
};
